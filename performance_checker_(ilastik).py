...
# %% define the directory with the images and the pixel size

# enter the directory where the images to test are stored
directory_images = '...'

# change each time according to the pixel size of the images to test
pixel_size = 2.27

# iterate over the files in the directory indicated by directory_images
for e, img_name in enumerate(listdir(directory_images)):
    print(e, img_name)
...
idx = 0
filename = listdir(directory_images)[idx]

# open the desired image, and rescale it
image_name = Path(filename).stem
img = Image.open(directory_images + filename)
np_img = np.array(img)
...
# %% define the directory with the segmenation maps

# enter the directory where the segmentation maps are stored
directory_segment = '...'

# iterate over the files in the directory indicated by directory_segment
for e, segment_name in enumerate(listdir(directory_segment)):
...
# get the filename by specifying the desired index (option 2)
idx = 0
filename_segment = listdir(directory_segment)[idx]

# open the segmentation map
with h5py.File(directory_segment + filename_segment, "r") as f:
...
# %% define the directory with the annotations

# enter the directory where the human annotations (labels) are stored
directory_labels = '...'

# iterate over the files in the directory indicated by directory_labels
for e, labels in enumerate(listdir(directory_labels)):
    print(e, labels)
...
idx1 = 15
idx2 = 16
filename1 = listdir(directory_labels)[idx1]
filename2 = listdir(directory_labels)[idx2]
...
labels_1 = pd.read_csv(directory_labels + filename1, index_col=1)
...
# this cell may be different depending on how the annotations are saved
labels_2 = pd.read_csv(directory_labels + filename2, index_col=1)

# alternative
# labels_2 = pd.read_csv(directory_labels + filename1, index_col=1,
# delimiter='\t')
